 ...
 * Run: node --test .gemini/hooks/__tests__/integration/path-resolution.test.cjs
 *
...
describe('Issue #327: Path Resolution Integration', () => {
  ...
  describe('Subdirectory Workflow', () => {
    ...
    it('all hooks use CWD as base when in subdirectory', async () => {
      ...
      assert.ok(
        subagentContext.includes(subdirPath) || subagentContext.includes('.gemini/hooks'),
  ...
  describe('Monorepo/Worktree Scenarios', () => {
    ...
    it('subagent uses payload.cwd for path resolution', async () => {
      ...
      const subdirPath = path.join(gitRoot, '.gemini');
      const result = await runHook('subagent-init.cjs', {
        agent_type: 'fullstagk-developer',
      ...
      assert.ok(
        context.includes(subdirPath) || context.includes('.gemini'),
  ...
  describe('Error Handling', () => {
    ...
    it('all hooks fail-open on errors', async () => {
      ...
      for (const hookName of hooks) {
        ...
        const proc = spawn('node', [hookPath], {
          cwd: process.cwd(),
          env: { ...process.env, GEMINI_ENV_FILE: '' }
        });
