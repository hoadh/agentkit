 ...
 * Run: node --test .gemini/hooks/__tests__/session-init.test.cjs
 *
...
function runHook(inputData) {
  return new Promise((resolve, reject) => {
    const proc = spawn('node', [HOOK_PATH], {
      ...
      env: {
        ...process.env,
        // Unset GEMINI_ENV_FILE to avoid writing env vars during tests
        GEMINI_ENV_FILE: ''
      }
    });
...
describe('session-init.cjs', () => {
  ...
  describe('Issue #291: Git Root Path Resolution', () => {
    ...
    it('shows subdirectory info when CWD differs from git root (Issue #327)', async () => {
      ...
      // Use .gemini/hooks as subdirectory (guaranteed to exist)
      const subdirPath = require('path').join(gitRoot, '.gemini', 'hooks');
