 ...
 * Run: node --test .gemini/hooks/__tests__/task-completed-handler.test.cjs
 */
...
function createTestTeam(baseDir, teamName, tasks) {
  const taskDir = path.join(baseDir, '.gemini', 'tasks', teamName);
...
describe('task-completed-handler.cjs', () => {
  ...
  describe('Error resilience', () => {
    ...
    it('handles corrupted task JSON gracefully', async () => {
      ...
      fs.mkdirSync(tmpDir, { recursive: true });
      try {
        const taskDir = path.join(tmpDir, '.gemini', 'tasks', 'bad-team');
        fs.mkdirSync(taskDir, { recursive: true });
