 ...
 *
 * Fires when Plan subagent completes. Reminds to invoke skill:cook --auto before implementation.
...
try {
  ...
  try {
    ...
    console.log('MUST invoke skill:cook --auto skill before implementing the plan');
    if (planPath) {
      const planMdPath = path.join(planPath, 'plan.md');
      console.log(`Best Practice: Run /clear then skill:cook ${planMdPath}`);
    } else {
      // Fallback when plan path unavailable
      console.log('Best Practice: Run /clear then skill:cook {full-absolute-path-to-plan.md}');
    }
