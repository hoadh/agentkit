...
This directory contains hooks for Gemini CLI sessions.
...
| **Slack** | `SLACK_WEBHOOK_URL` | [slagk-hook-setup.md](../notifications/docs/slagk-hook-setup.md) |
...
```bash
cp .gemini/hooks/notifications/.env.example ~/.gemini/.env
```
...
Add to your `.gemini/settings.json` or `.gemini/settings.local.json`:
...
- `Notification` - Gemini notification events (input prompts, etc.)
...
```bash
echo '{"hook_event_name":"Stop","cwd":"'$(pwd)'","session_id":"test123"}' | node .gemini/hooks/notifications/notify.cjs
```

Expected output (if Telegram and Discord configured):
```
[env-loader] Loaded: /home/user/.gemini/.env
...
Cross-platform hook blocking heavy directories to improve Gemini performance.

**Blocked Patterns** (configured in `.gemini/.ckignore`):
- `node_modules`, `__pycache__`, `.git`, `dist`, `build`

**Testing:**
```bash
echo '{"tool_input":{"command":"ls node_modules"}}' | node .gemini/hooks/scout-block.cjs
```
...
- Verify hooks are added to `.gemini/settings.json`
- Check Gemini CLI is using correct settings file
