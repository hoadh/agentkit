...
describe('context-builder.cjs', () => {
  ...
  describe('resolveRulesPath()', () => {
    ...
    it('returns null when file does not exist anywhere', () => {
      tempDir = createTempDir(['.gemini']);
      process.chdir(tempDir);

      // Use a unique filename that won't exist in global ~/.gemini/
    ...
    it('finds file in rules/ directory (new location)', () => {
      ...
      assert.strictEqual(result, '.gemini/rules/development-rules.md',
        'Should find file in rules/ directory');
    });

    it('falls back to workflows/ when rules/ does not exist', () => {
      tempDir = createTempDir(['.gemini/workflows']);
      createTestFile(path.join(tempDir, '.gemini/workflows'), 'development-rules.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('development-rules.md');
      assert.strictEqual(result, '.gemini/workflows/development-rules.md',
        'Should fall back to workflows/ directory');
    });

    it('prefers rules/ over workflows/ when both exist', () => {
      tempDir = createTempDir(['.gemini/rules', '.gemini/workflows']);
      createTestFile(path.join(tempDir, '.gemini/rules'), 'development-rules.md', '# Rules version\n');
      createTestFile(path.join(tempDir, '.gemini/workflows'), 'development-rules.md', '# Workflows version\n');
      process.chdir(tempDir);
    ...
    it('finds file in workflows/ when rules/ exists but file is only in workflows/', () => {
      ...
      createTestFile(path.join(tempDir, '.gemini/workflows'), 'legacy-file.md');
      process.chdir(tempDir);

      const result = contextBuilder.resolveRulesPath('legacy-file.md');
      assert.strictEqual(result, '.gemini/workflows/legacy-file.md',
  ...
  describe('resolveWorkflowPath alias', () => {
    ...
    it('resolveWorkflowPath works identically to resolveRulesPath', () => {
      const tempDir = createTempDir(['.gemini/rules']);
      createTestFile(path.join(tempDir, '.gemini/rules'), 'test.md');
  ...
  describe('Global path resolution', () => {
    ...
    it('checks global ~/.gemini/rules/ path', () => {
      ...
      const tempDir = createTempDir(['.gemini']);
      const originalCwd = process.cwd();
      process.chdir(tempDir);
      ...
      try {
        ...
        // Result depends on whether global ~/.gemini/rules exists
  ...
  describe('GEMINI.md reference resolution', () => {
    ...
    it('resolves @rules/ references correctly', () => {
      ...
      createTestFile(path.join(tempDir, '.gemini/rules'), 'development-rules.md');
      createTestFile(path.join(tempDir, '.gemini/rules'), 'orchestration-protocol.md');
    ...
    it('resolves legacy @workflows/ references via fallback', () => {
      ...
      tempDir = createTempDir(['.gemini/workflows']);
      createTestFile(path.join(tempDir, '.gemini/workflows'), 'primary-workflow.md');
      process.chdir(tempDir);
