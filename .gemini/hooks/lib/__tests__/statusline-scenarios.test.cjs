...
const USAGE_CACHE_PATH = path.join(os.tmpdir(), `gk-usage-limits-cache-statusline-test-${process.pid}.json`);
...
function createTempConfigProject(mode) {
  ...
  const ckDir = path.join(tmpDir, '.gemini');
...
async function main() {
  ...
  await test('Mode compact keeps output to two lines', async () => {
    ...
    try {
      ...
      assertContains(stdout, 'Gemini', 'Compact mode should include model');
    } finally {
      fs.rmSync(tmpDir, { recursive: true, force: true });
    }
  });
  ...
  await test('Default/full mode includes model and path', async () => {
    ...
    const { stdout } = result;
    assertContains(stdout, 'Gemini', 'Full mode should include model');
