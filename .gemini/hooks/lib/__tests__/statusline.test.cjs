 ...
 * Run: node .gemini/hooks/lib/__tests__/statusline.test.cjs
 */
      ...
      content: [
        {
          ...
          id: 'agent-1',
          name: 'Task',
          input: { subagent_type: 'researcher', model: 'gemini-opus', description: 'Research topic' }
        }
      ]
    }
  },
...
test('parseTranscript tracks agents correctly', async () => {
  ...
  assertEquals(agent.model, 'gemini-opus', 'Should capture agent model');
});
...
test('countConfigs: returns object with expected properties', () => {
  ...
  assertTrue('geminiMdCount' in result, 'Should have geminiMdCount');
...
test('countConfigs: all counts are numbers', () => {
  const result = countConfigs('/tmp');
  assertEquals(typeof result.geminiMdCount, 'number', 'geminiMdCount should be number');
...
test('getGitInfo timeout does not cache null result', () => {
  ...
  const cachePath = path.join(os.tmpdir(), `gk-git-cache-${hash}.json`);
