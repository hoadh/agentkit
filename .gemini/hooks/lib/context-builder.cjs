 ...
 *
 * Extracted from dev-rules-reminder.cjs for reuse in both Gemini hooks and OpenCode plugins.
 ...
 * @param {string} [configDirName='.gemini'] - Config directory name
 * @returns {string|null} Resolved path or null
 */
function resolveRulesPath(filename, configDirName = '.gemini') {
  // Try rules/ first (new location)
  const localRulesPath = path.join(process.cwd(), configDirName, 'rules', filename);
  const globalRulesPath = path.join(os.homedir(), '.gemini', 'rules', filename);

  if (fs.existsSync(localRulesPath)) return `${configDirName}/rules/${filename}`;
  if (fs.existsSync(globalRulesPath)) return `~/.gemini/rules/${filename}`;
  ...
  const globalWorkflowsPath = path.join(os.homedir(), '.gemini', 'workflows', filename);

  if (fs.existsSync(localWorkflowsPath)) return `${configDirName}/workflows/${filename}`;
  if (fs.existsSync(globalWorkflowsPath)) return `~/.gemini/workflows/${filename}`;
 ...
 * @returns {string|null} Resolved path or null
 */
function resolveScriptPath(filename, configDirName = '.gemini') {
 ...
 */
function resolveSkillsVenv(configDirName = '.gemini') {
  ...
  const globalVenv = path.join(os.homedir(), '.gemini', 'skills', '.venv', venvBin, pythonExe);
  ...
  if (fs.existsSync(globalVenv)) {
    return isWindows
      ? '~\\.gemini\\skills\\.venv\\Scripts\\python.exe'
      : '~/.gemini/skills/.venv/bin/python3';
  }
  return null;
}
...
function buildRulesSection({ devRulesPath, skillsVenv, plansPath, docsPath }) {
  ...
  // Issue #476: Use absolute paths to prevent LLM confusion in multi-GEMINI.md projects
  ...
  if (skillsVenv) {
    lines.push(`- Python scripts in .gemini/skills/: Use \`${skillsVenv}\``);
  }
 ...
 * @param {string} [params.configDirName='.gemini'] - Config directory name
 ...
 * }}
 */
function buildReminderContext({ sessionId, config, staticEnv, configDirName = '.gemini', baseDir } = {}) {
