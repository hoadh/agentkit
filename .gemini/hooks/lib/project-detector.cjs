 ...
 *
 * Extracted from session-init.cjs for reuse in both Gemini hooks and OpenCode plugins.
...
function getCodingLevelGuidelines(level, configDir) {
  ...
  const basePath = configDir || path.join(process.cwd(), '.gemini');
...
function buildStaticEnv(configDir) {
  return {
    ...
    locale: process.env.LANG || '',
    timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
    configDir: configDir || path.join(process.cwd(), '.gemini')
  };
}
