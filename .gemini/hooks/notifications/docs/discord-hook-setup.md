...
Get Discord notifications when Gemini CLI sessions complete.
...
3. Click **"New Webhook"**
4. Configure webhook:
   - **Name:** `Gemini CLI Bot` (or your preference)
...
**Option B: `.gemini/.env`** (project-level override):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```

**Option C: `.gemini/hooks/.env`** (hook-specific):
```bash
DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/YOUR_WEBHOOK_ID/YOUR_WEBHOOK_TOKEN
```
...
**Implementation:** `.gemini/hooks/notifications/notify.cjs` + `providers/discord.cjs`
...
**Gemini CLI Documentation:** https://docs.gemini.com/gemini-code
