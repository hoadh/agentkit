...
Get Telegram notifications when Gemini CLI sessions complete.

## Quick Start

### 1. Set Environment Variables

Add to `~/.gemini/.env` (global) or `.gemini/.env` (project):
...
Add to your `.gemini/settings.json` (project) or `~/.gemini/settings.json` (global):
    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "node .gemini/hooks/notifications/notify.cjs"
          }
        ]
      }
    ]
...
echo '{"hook_event_name":"Stop","cwd":"'"$(pwd)"'","session_id":"test123"}' | \
  node .gemini/hooks/notifications/notify.cjs
```
...
3. Follow the prompts:
   ...
   You: Gemini CLI Notifier

   BotFather: Good. Now let's choose a username for your bot.
   You: geminicode_notifier_bot
   ```
...
2. **.gemini/.env** - Project-level Gemini configuration
3. **.gemini/hooks/.env** - Hook-specific configuration
...
#### Option C: `.gemini/.env` (Project-Level Override)

For project-specific Gemini configuration:
...
#### Option D: `.gemini/hooks/.env` (Hook-Specific)
...
### 4. Configure Gemini CLI Hook

Hooks are configured in `.gemini/settings.local.json`:
...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "node ${GEMINI_PROJECT_DIR}/.gemini/hooks/notifications/notify.cjs"
      }]
    }],
    "SubagentStop": [{
      "hooks": [{
...
- `"Stop"`: Triggers when main Gemini CLI session ends
- `"SubagentStop"`: Triggers when specialized subagents complete (planner, tester, etc.)
- `${GEMINI_PROJECT_DIR}`: Environment variable for project directory path
...
> **Note:** Gemini CLI hooks use snake_case field names. The `Stop` hook does not include tool usage data.
...
**Triggered when:** Main Gemini CLI session ends (user stops Gemini or task completes)
...
📍 Location: `/Users/user/projects/geminikit-engineer`
```
...
**Cause:** Gemini CLI hook configuration incorrect

**Solutions:**

1. **Verify `.gemini/settings.local.json` exists and is valid JSON:**
   ```bash
   cat .gemini/settings.local.json | node -e "process.stdin.resume();let d='';process.stdin.on('data',c=>d+=c);process.stdin.on('end',()=>console.log(JSON.parse(d)))"
   ```
...
> **Note:** Gemini CLI hooks use snake_case field names per the official API.
...
**Implementation:** `.gemini/hooks/notifications/notify.cjs` + `providers/telegram.cjs`

**Configuration:** `.gemini/settings.local.json`
...
**Gemini CLI Hooks:** https://docs.gemini.com/gemini-code/hooks
