...
function getTitle(hookType) {
  switch (hookType) {
    case 'Stop':
      return 'Gemini CLI Session Complete';
    case 'SubagentStop':
      return 'Gemini CLI Subagent Complete';
    case 'AskUserPrompt':
      return 'Gemini CLI Needs Input';
    default:
      return 'Gemini CLI Event';
  }
}
...
function formatMessage(input) {
  ...
  return {
    text: `Gemini CLI: ${hookType} in ${projectName}`, // Fallback required
