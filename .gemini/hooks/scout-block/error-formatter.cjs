 ...
 * @returns {string}
 */
function formatConfigPath(geminiDir) {
  if (geminiDir) {
    return path.join(geminiDir, '.ckignore');
  }
  return '.gemini/.ckignore';
}
 ...
 * @param {string} details.geminiDir - Path to .gemini directory
 * @returns {string}
 */
function formatBlockedError(details) {
  const { path: blockedPath, pattern, tool, geminiDir } = details;
  const configPath = formatConfigPath(geminiDir);
