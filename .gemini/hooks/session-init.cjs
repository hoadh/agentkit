...
function cleanupOrphanedShadowedSkills() {
  const shadowedDir = path.join(process.cwd(), '.gemini', 'skills', '.shadowed');
 ...
 * Scans ~/.gemini/teams/ for active team configs and checks membership.
 * Note: Returns first team found — Gemini CLI supports one team per session.
 * Note: Team lifecycle (creation/cleanup) is managed by Gemini CLI, not this hook.
 * @returns {{ teamName: string, memberCount: number } | null}
 */
function detectAgentTeam() {
  try {
    const teamsDir = path.join(os.homedir(), '.gemini', 'teams');
...
async function main() {
  ...
  try {
    ...
    // Extract task list ID for Gemini CLI Tasks coordination (shared helper)
    ...
    if (envFile) {
      ...
      // Gemini CLI Tasks integration - enables multi-session/subagent coordination
      // Task list ID = plan directory name (shared across all sessions working on same plan)
      if (taskListId) {
        writeEnv(envFile, 'GEMINI_CODE_TASK_LIST_ID', taskListId);
      }
      ...
      writeEnv(envFile, 'CK_GEMINI_SETTINGS_DIR', staticEnv.geminiSettingsDir);
      ...
      // Plan validation config (for skill:plan validate, skill:plan --hard, skill:plan --parallel)
    ...
    if (hasCleanup) {
      ...
      if (shadowedCleanup.kept.length > 0) {
        ...
        console.log(`    Review .gemini/skills/.shadowed/ and merge changes manually.`);
      }
    }
    ...
    if (teamInfo) {
      ...
      console.log(`    Team config: ~/.gemini/teams/${teamInfo.teamName}/config.json`);
      console.log(`    Use skill:team skill for orchestration templates.`);
    }
    ...
    // This warning reminds Gemini to verify if user approval was pending before proceeding.
    // Upstream bug: Gemini CLI CLI should preserve pending interactive state during compaction.
    if (source === 'compact') {
