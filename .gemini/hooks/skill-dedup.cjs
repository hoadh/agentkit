 ...
 * when multiple Gemini CLI sessions run in the same directory.
 */
...
function getDefaultPaths() {
  const globalDir = path.join(os.homedir(), '.gemini', 'skills');
  const localDir = path.join(process.cwd(), '.gemini', 'skills');
