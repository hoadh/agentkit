...
async function main() {
  ...
  // Config toggle tests - requires temp directory with .gemini/.ck.json
  console.log('\n\x1b[1m--- Config Toggle (privacyBlock setting) ---\x1b[0m');
  const os = require('os');
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'privacy-test-'));
  const tmpGeminiDir = path.join(tmpDir, '.gemini');
  fs.mkdirSync(tmpGeminiDir, { recursive: true });

  for (const test of configToggleTests) {
    // Write test config
    fs.writeFileSync(
      path.join(tmpGeminiDir, '.ck.json'),
