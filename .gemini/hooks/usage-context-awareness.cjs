 ...
 *
 * Fetches Gemini CLI usage limits from Google OAuth API and writes to cache.
...
/**
 * Get Gemini OAuth credentials (cross-platform)
 */
function getGeminiCredentials() {
	// macOS: Try Keychain first
	if (os.platform() === "darwin") {
		try {
			const result = execSync('security find-generic-password -s "Gemini CLI-credentials" -w', {
				timeout: 5000,
				encoding: "utf-8",
				stdio: ["pipe", "pipe", "ignore"],
			}).trim();
			const parsed = JSON.parse(result);
			if (parsed.geminiAiOauth?.accessToken) {
				return parsed.geminiAiOauth.accessToken;
			}
		} catch {
	...
	try {
		...
		return parsed.geminiAiOauth?.accessToken || null;
	} catch {
		return null;
	}
}
...
async function fetchAndCacheUsageLimits() {
	...
	try {
		const response = await fetch("https://api.google.com/api/oauth/usage", {
			method: "GET",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
				Authorization: `Bearer ${token}`,
				"google-beta": "oauth-2025-04-20",
				"User-Agent": "geminikit-engineer/1.0",
			},
		});
