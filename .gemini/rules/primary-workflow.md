...
- Use `skill:preview --explain <topic>` to generate visual explanation with ASCII + Mermaid
- Use `skill:preview --diagram <topic>` for architecture and data flow diagrams
- Use `skill:preview --slides <topic>` for step-by-step walkthroughs
- Use `skill:preview --ascii <topic>` for terminal-friendly output only
- **HTML mode** (add `--html` for self-contained HTML pages, opens directly in browser):
  - `skill:preview --html --explain <topic>` — publication-quality HTML explanation
  - `skill:preview --html --diagram <topic>` — interactive HTML diagram with zoom controls
  - `skill:preview --html --slides <topic>` — magazine-quality slide deck
  - `skill:preview --html --diff [ref]` — visual diff review
  - `skill:preview --html --plan-review` — plan vs codebase comparison
