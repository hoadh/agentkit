...
Centralized utility scripts for Gemini CLI skills.
...
Centralized environment variable resolver that follows Gemini CLI's hierarchy.
...
python ~/.gemini/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal

# With verbose output
python ~/.gemini/scripts/resolve_env.py GEMINI_API_KEY --skill ai-multimodal --verbose

# Find all locations where variable is defined
python ~/.gemini/scripts/resolve_env.py GEMINI_API_KEY --find-all

# Show hierarchy for a skill
python ~/.gemini/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal

# Export format for shell sourcing
eval $(python ~/.gemini/scripts/resolve_env.py GEMINI_API_KEY --export)
```
...
if not api_key:
    ...
    print("Run: python ~/.gemini/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
    sys.exit(1)
...
python ~/.gemini/scripts/resolve_env.py TEST_VAR --verbose
