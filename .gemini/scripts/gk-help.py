...
Usage:
    ...
    python gk-help.py plan:fast          # Command details
...
def emit_output_type(output_type: str) -> None:
    ...
    print(f"@GK_OUTPUT_TYPE:{output_type}")
    print()
...
TASK_MAPPINGS = {
    ...
    "config": ["config", "configure", "settings", "gemini.json", ".gemini.json", "setup", "locale", "language", "paths"],
...
CATEGORY_GUIDES = {
    ...
    "notifications": {
        ...
        "workflow": [
            ...
            ("3. Test", "`echo '{\"hook_event_name\":\"Stop\"}' | node hooks/notifications/notify.cjs`"),
        ],
        "tip": """Add to settings.json:
```json
"Stop": [{"matcher": "*", "hooks": [{"type": "command", "command": "node hooks/notifications/notify.cjs"}]}]
```
Docs: `hooks/notifications/docs/`""",
    },
}


def detect_prefix(commands_dir: Path) -> str:
    """Detect if commands use /gk: prefix based on directory structure."""
    gk_commands_dir = commands_dir / "gk"
    return "gk:" if gk_commands_dir.exists() and gk_commands_dir.is_dir() else ""
...
def discover_commands(commands_dir: Path, prefix: str) -> dict:
    ...
    # Scan all .toml files (Gemini CLI format)
    for toml_file in commands_dir.rglob("*.toml"):
        # Skip non-command files
        rel_path = toml_file.relative_to(commands_dir)
        parts = rel_path.parts

        # Get command name from path
        # e.g., fix:fast.toml -> fix:fast
        # e.g., plan.toml -> plan
        # Handle cases where colon is already in filename
        filename = parts[-1].replace('.toml', '')
        if len(parts) == 1:
            cmd_name = filename
            category = filename.split(':')[0] if ':' in filename else "core"
        else:
            category = parts[0]
            cmd_name = ':'.join([p.replace('.toml', '') for p in parts])

        # For Gemini CLI, we need to parse TOML to get description
        # But for efficiency, we can just use regex for now if we don't want to add toml dependency
        try:
...
def show_command(data: dict, command: str, prefix: str) -> None:
    ...
    search = command.lower().replace("/gk:", "").replace("/", "").replace(":", "")

    found = None
    for cmds in commands.values():
        for cmd in cmds:
            # Normalize command name for comparison
            name = cmd["name"].lower().replace("/gk:", "").replace("/", "").replace(":", "")
            if name == search:
                found = cmd
                break
        if found:
...
def show_config_guide() -> None:
    ...
    print("- Local: `./.gemini/.agent.json` (project overrides)")
    print()
    ...
    print("## Quick Start")
    print()
    print("**Global config** (`~/.gemini/.agent.json`) - your preferences:")
    print("```json")
    print('{')
    print('  "locale": {')
    ...
    print("```")
    print()
    print("**Local override** (`./.gemini/.agent.json`) - project-specific:")
    print("```json")
    print('{')
    ...
    print("```bash")
    print("# ~/.gemini/.agent.json - applies everywhere")
    ...
    print("# Local (.gemini/.agent.json): issuePrefix = \"JIRA-\"")
...
def show_coding_level_guide() -> None:
    ...
    print("- Global: `~/.gemini/.agent.json` - personal preference")
    print("- Local: `./.gemini/.agent.json` - project override")
    print()
    print("---")
    print()
    print("## How It Works")
    print()
    print("1. SessionStart hook reads `codingLevel` from `.agent.json`")
...
def main():
    ...
    if not data["commands"]:
        print(f"No commands found in {commands_dir}")
        sys.exit(1)
    ...
    if input_str.lower() in ["config", "configuration", ".gemini.json", "gemini.json"]:
        show_config_guide()
        return
