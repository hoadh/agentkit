...
This guide explains how to install dependencies for Gemini CLI skills.
...
pip install -r .gemini/skills/ai-multimodal/scripts/requirements.txt
...
```bash
cd .gemini/skills/ai-multimodal/scripts
...
**ai-multimodal** (`.gemini/skills/ai-multimodal/scripts/requirements.txt`):
...
```bash
cd .gemini/skills/{skill-name}/scripts
...
2. **`.gemini/skills/{skill-name}/.env`** (skill-specific config)
3. **`.gemini/skills/.env`** (shared skills config)
4. **`.gemini/.env`** (global Gemini config)
...
```bash
chmod +x .gemini/skills/*/scripts/*.py
```
...
# Run all tests
pytest .gemini/skills/*/scripts/tests/ -v

# Check coverage across all skills
pytest .gemini/skills/*/scripts/tests/ --cov=.gemini/skills --cov-report=html
```
