...
GEMINI_ROOT = Path(__file__).parent.parent.parent.parent
sys.path.insert(0, str(GEMINI_ROOT / 'scripts'))
try:
...
def find_api_key() -> Optional[str]:
    ...
    Uses ~/.gemini/scripts/resolve_env.py for consistent resolution across all skills.
    Falls back to local resolution if centralized resolver not available.

    Priority order (highest to lowest):
    1. process.env (runtime environment variables)
    2. PROJECT/.gemini/skills/ai-multimodal/.env (skill-specific)
    3. PROJECT/.gemini/skills/.env (shared skills)
    4. PROJECT/.gemini/.env (project global)
    5. ~/.gemini/skills/ai-multimodal/.env (user skill-specific)
    6. ~/.gemini/skills/.env (user shared)
    7. ~/.gemini/.env (user global)
    """
                ...
                for i, part in enumerate(response.candidates[0].content.parts):
                    if part.inline_data:
                        ...
                        if file_path:
                            ...
                            # Find project root (look for .git or .gemini directory)
                            script_dir = Path(__file__).parent
                            project_root = script_dir
                            for parent in [script_dir] + list(script_dir.parents):
                                if (parent / '.git').exists() or (parent / '.gemini').exists():
...
def batch_process(
    ...
    if not api_key:
        ...
        print("2. Show hierarchy: python ~/.gemini/scripts/resolve_env.py --show-hierarchy --skill ai-multimodal")
        print("3. Quick setup: export GEMINI_API_KEY='your-key'")
        print("4. Create .env: cd ~/.gemini/skills/ai-multimodal && cp .env.example .env")
