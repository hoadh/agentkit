...
```
skill:bootstrap <user-requirements>
```
...
**Example:**
```
skill:bootstrap "Build a SaaS dashboard with auth" --fast
skill:bootstrap "E-commerce platform with Stripe" --parallel
```
...
- `--parallel` → `skill:plan --parallel <requirements>` (file ownership + dependency graph)
...
- `--parallel` → `skill:cook --parallel <plan-path>` (multi-agent execution)
...
- Run `skill:journal` to write a concise technical journal entry upon completion
