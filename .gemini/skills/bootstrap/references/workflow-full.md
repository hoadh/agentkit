...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `gk-ai-multimodal` skill generation
3. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `gk-ai-multimodal` skill
5. Screenshot wireframes with `gk-chrome-devtools` → save to `./docs/wireframes/`
...
Activate **gk-plan** skill: `skill:plan --hard <requirements>`
...
Activate **gk-cook** skill: `skill:cook <plan-path>` (interactive mode — review gates at each step)
