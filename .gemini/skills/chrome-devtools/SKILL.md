...
SKILL_DIR=""
if [ -d ".gemini/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR=".gemini/skills/chrome-devtools/scripts"
elif [ -d "$HOME/.gemini/skills/chrome-devtools/scripts" ]; then
  SKILL_DIR="$HOME/.gemini/skills/chrome-devtools/scripts"
fi
...
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output ./.gemini/chrome-devtools/snapshots/page.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
Store snapshots for analysis in `<project>/.gemini/chrome-devtools/snapshots/`:

```bash
# Create snapshots directory
mkdir -p .gemini/chrome-devtools/snapshots

# Capture and store with timestamp
SESSION="$(date +%Y%m%d-%H%M%S)"
node "$SKILL_DIR/aria-snapshot.js" --url https://example.com --output .gemini/chrome-devtools/snapshots/$SESSION.yaml
```
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
All in `.gemini/skills/chrome-devtools/scripts/`:
...
Skills can exist in **project-scope** or **user-scope**. Priority: project-scope > user-scope.
For complex automation, write scripts to `<project>/.gemini/chrome-devtools/tmp/`:

```bash
# Create tmp directory for test scripts
mkdir -p $SKILL_DIR/.gemini/chrome-devtools/tmp

# Write a test script
cat > $SKILL_DIR/.gemini/chrome-devtools/tmp/login-test.js << 'EOF'
...
# Run the test
node $SKILL_DIR/.gemini/chrome-devtools/tmp/login-test.js
```
...
Store screenshots for analysis in `<project>/.gemini/chrome-devtools/screenshots/`:

```bash
# Basic screenshot
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./.gemini/chrome-devtools/screenshots/page.png

# Full page
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./.gemini/chrome-devtools/screenshots/page.png --full-page true

# Specific element
node "$SKILL_DIR/screenshot.js" --url https://example.com --selector ".main-content" --output ./.gemini/chrome-devtools/screenshots/element.png
```
...
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./.gemini/chrome-devtools/screenshots/page.png --max-size 3

# Disable compression
node "$SKILL_DIR/screenshot.js" --url https://example.com --output ./.gemini/chrome-devtools/screenshots/page.png --no-compress
```
...
Store logs for analysis in `<project>/.gemini/chrome-devtools/logs/<session>/`:

```bash
# Create session directory
SESSION="$(date +%Y%m%d-%H%M%S)"
mkdir -p .gemini/chrome-devtools/logs/$SESSION

# Capture and store
node "$SKILL_DIR/console.js" --url https://example.com --duration 10000 > .gemini/chrome-devtools/logs/$SESSION/console.json
node "$SKILL_DIR/network.js" --url https://example.com > .gemini/chrome-devtools/logs/$SESSION/network.json

# View errors
jq '.messages[] | select(.type=="error")' .gemini/chrome-devtools/logs/$SESSION/console.json
```
...
node "$SKILL_DIR/screenshot.js" --output ./.gemini/skills/chrome-devtools/screenshots/debug.png
