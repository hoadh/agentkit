...
Monitor usage limits and context window utilization in real-time to optimize Gemini CLI sessions.
...
```
GET https://api.google.com/api/oauth/usage
```

### Authentication

Requires OAuth Bearer token with `google-beta: oauth-2025-04-20` header.

### Credential Locations

| Platform | Method | Location |
|----------|--------|----------|
| macOS | Keychain | `Gemini CLI-credentials` |
| Windows | File | `%USERPROFILE%\.gemini\.credentials.json` |
| Linux | File | `~/.gemini/.credentials.json` |
...
### Hook Settings (`.gemini/settings.json`)
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [{
          "type": "command",
          "command": "node .gemini/hooks/usage-context-awareness.cjs"
        }]
      }
    ]
  }
...
|-------|-------|----------|
| No usage limits shown | No OAuth token | Run `gemini login` |
