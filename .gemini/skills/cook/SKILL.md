...
**Example:**
```
skill:cook "Add user authentication to the app" --fast
skill:cook path/to/plan.md --auto
```
...
**Gemini Tasks:** Utilize `TaskCreate`, `TaskUpdate`, `TaskGet`, `TaskList` during implementation step. **Fallback:** These are CLI-only tools — unavailable in VSCode extension. If they error, use `TodoWrite` for progress tracking instead.
...
- **Finalize (MANDATORY - never skip):**
  ...
  3. `TaskUpdate` → mark all Gemini Tasks complete after sync-back verification (skip if Task tools unavailable)
...
| Scout | `gk-scout` | Optional in code |
