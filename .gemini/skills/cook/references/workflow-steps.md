...
- Use `skill:scout ext` or `scout` agent for codebase search
...
**Fast:**
- Use `skill:plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `skill:plan --parallel` for dependency graph + file ownership matrix
...
- Use `AskUserQuestion` to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `skill:plan validate` skill invocation
...
**Parallel mode:**
- Utilize all tools of Gemini Tasks: `TaskCreate`, `TaskUpdate`, `TaskGet` and `TaskList`
- Launch multiple `fullstagk-developer` agents
...
1. **MUST** spawn these subagents in parallel:
   - `Task(subagent_type="project-manager", prompt="Run full sync-back for [plan-path]: reconcile all completed Gemini Tasks with all phase files, backfill stale completed checkboxes across every phase, then update plan.md frontmatter/table progress. Do NOT only mark current phase.", description="Update plan")`
...
3. Use `TaskUpdate` to mark Gemini Tasks complete after sync-back confirmation.
...
- Use `TaskCreate` to create Gemini Tasks for each unchecked item with priority order and dependencies (or `TodoWrite` if Task tools unavailable).
- Use `TaskUpdate` to mark Gemini Tasks `in_progress` when picking up a task (skip if Task tools unavailable).
- Use `TaskUpdate` to mark Gemini Tasks `complete` immediately after finalizing the task (skip if Task tools unavailable).
