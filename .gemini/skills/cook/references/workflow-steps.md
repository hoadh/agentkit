...
**IMPORTANT:**
- Read plan overview and all phases, use `TaskCreate` to create Gemini Tasks for each unchecked item.
...
2. Use `TaskUpdate` to mark Gemini Tasks complete immediately.
...
- Use `TaskCreate` to create Gemini Tasks for each unchecked item with priority order and dependencies.
- Use `TaskUpdate` to mark Gemini Tasks `in_progress` when picking up a task.
- Use `TaskUpdate` to mark Gemini Tasks `complete` immediately after finalizing the task.
