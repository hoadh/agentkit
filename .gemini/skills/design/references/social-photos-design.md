...
Invoke `project-management` skill to create persistent TODO tasks via Gemini's native task orchestration. Break down into:
...
3. **Randomly invoke ONE of:** `skill:ui-ux-pro-max` OR `skill:frontend-design` — for layout, hierarchy, visual balance. Pick one at random each run for design variety.
