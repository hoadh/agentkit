...
1. Scan the codebase and calculate the number of files with LOC in each directory (skip `.gemini`, `.opencode`, `.git`, `tests`, `node_modules`, `__pycache__`, `secrets`, etc.)
2. Target directories **that actually exist** - adapt to project structure
3. Activate `gk-scout` skill to explore the code base and return detailed summary reports
...
1. Run: `node .gemini/scripts/validate-docs.cjs docs/`
