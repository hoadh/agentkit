...
- `--parallel` - Activate parallel mode: route to parallel `fullstagk-developer` agents per issue
...
| **Parallel** | 2+ independent issues OR `--parallel` flag | Parallel `fullstagk-developer` agents |

**Task Orchestration (Moderate+ only):** After classifying, create native Gemini Tasks for all phases upfront with dependencies. See `references/task-orchestration.md`.
...
3. `TaskUpdate` → mark ALL Gemini Tasks `completed` (skip if Task tools unavailable)
4. Ask user if they want to commit via `git-manager` subagent
5. Run `skill:journal` to write a concise technical journal entry upon completion
...
- `references/task-orchestration.md` - Native Gemini Task patterns for moderate+ workflows
