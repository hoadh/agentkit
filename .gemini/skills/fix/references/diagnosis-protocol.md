...
Read, don't assume. Use `gk-debug` (systematic-debugging Phase 1).
...
Activate `gk-sequential-thinking` skill. Form hypotheses through structured reasoning.
...
Use `gk-debug` (root-cause-tracing technique). Trace backward:
...
1. Auto-activate `gk-problem-solving` skill
