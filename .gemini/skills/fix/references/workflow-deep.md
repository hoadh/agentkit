...
Full pipeline with research, brainstorming, and planning for complex issues. Uses native Gemini Tasks with dependency chains.
...
**Mandatory:** Activate `gk-scout` skill or launch 2-3 `Explore` subagents in parallel:
```
...
2. Activate `gk-debug` skill (systematic-debugging + root-cause-tracing).
3. Activate `gk-sequential-thinking` — structured hypothesis formation.
4. Spawn parallel `Explore` subagents to test each hypothesis.
5. If 2+ hypotheses fail → auto-activate `gk-problem-solving`.
...
Implement per plan. Use `gk-context-engineering`, `gk-sequential-thinking`, `gk-problem-solving`.
...
- Activate `gk-project-management` for task sync-back, plan status updates, and progress tracking
...
|------|------------------|
| 1 | `gk-scout` OR parallel `Explore` subagents |
| 2 | `gk-debug`, `gk-sequential-thinking`, parallel `Explore`, (`gk-problem-solving` auto) |
| 3 | `researcher` (runs parallel with steps 1+2) |
| 4 | `gk-brainstorm` |
| 5 | `planner` |
| 6 | `gk-problem-solving`, `gk-sequential-thinking`, `gk-context-engineering` |
| 7 | `tester`, parallel `Bash` verification |
| 8 | `code-reviewer` |
| 9 | `gk-project-management`, `docs-manager`, `git-manager` |

**Rules:** Don't skip steps. Validate before proceeding. One phase at a time.
**Frontend:** Use `chrome`, `gk-chrome-devtools` or any relevant skills/tools to verify.
**Visual Assets:** Use `gk-ai-multimodal` for visual assets generation, analysis and verification.
