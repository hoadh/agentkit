...
Activate `gk-debug` skill. Activate `gk-sequential-thinking` for structured analysis.
...
|------|------------------|
| 1 | `gk-scout` (minimal) or direct file read |
| 2 | `gk-debug`, `gk-sequential-thinking` |
| 3 | Parallel `Bash` for verification |
| 4 | `code-reviewer` subagent |
| 5 | `git-manager` subagent |

**Extra:** `gk-context-engineering` if dealing with AI/LLM code
