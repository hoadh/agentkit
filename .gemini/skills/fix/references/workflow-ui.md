...
For fixing visual/UI issues. Requires design skills. Uses native Gemini Tasks for phase tracking.

## Required Skills (activate in order)
1. `gk-ui-ux-pro-max` - Design database (ALWAYS FIRST)
2. `gk-ui-ux-pro-max` - Design principles
3. `gk-frontend-design` - Implementation patterns

## Pre-fix Research
```bash
python3 .gemini/skills/ui-ux-pro-max/scripts/search.py "<product-type>" --domain product
python3 .gemini/skills/ui-ux-pro-max/scripts/search.py "<style>" --domain style
python3 .gemini/skills/ui-ux-pro-max/scripts/search.py "accessibility" --domain ux
```
...
Analyze screenshots/videos with `gk-ai-multimodal` skill.
...
Screenshot + `gk-ai-multimodal` analysis.
...
## Tips
- Use `gk-ai-multimodal` for generating visual assets
