...
Entry point for using the `gk-ai-multimodal` skill to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `gk-ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `gk-frontend-design` with `gk-ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `gk-chrome-devtools` Skill
Use `gk-chrome-devtools` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `gk-media-processing` Skill
