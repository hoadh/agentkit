...
Complete workflow for generating design-aligned visual assets using `gk-ai-multimodal` skill.
...
Use `gk-ai-multimodal` skill's image generation capabilities:
...
Use `gk-ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `gk-media-processing` skill for post-generation adjustments
