...
Visual verification of frontend implementations using Chrome MCP (Gemini Chrome Extension) or `gk-chrome-devtools` skill fallback.
...
Check if Chrome MCP server is available via `gk-mcp-management` skill or `ListMcpResourcesTool`:
...
When Chrome MCP is not configured, use `gk-chrome-devtools` skill (Puppeteer with bundled Chromium):
...
If `gk-chrome-devtools` skill is also unavailable, skip visual verification and note in report:
