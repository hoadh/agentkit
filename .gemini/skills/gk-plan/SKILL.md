...
6. **Red Team Review** → Run `skill:plan red-team {plan-path}` (hard/parallel/two modes)
7. **Post-Plan Validation** → Run `skill:plan validate {plan-path}` (hard/parallel/two modes)
8. **Hydrate Tasks** → Create Gemini Tasks from phases (default on, `--no-tasks` to skip)
...
After creating plan: `node .gemini/scripts/set-active-plan.cjs {plan-dir}`
...
|------------|-----------|---------|
| `skill:plan archive` | `references/archive-workflow.md` | Archive plans + write journal entries |
| `skill:plan red-team` | `references/red-team-workflow.md` | Adversarial plan review with hostile reviewers |
| `skill:plan validate` | `references/validate-workflow.md` | Validate plan with critical questions interview |
