...
4. **Context reminder:** `skill:cook --auto {absolute-plan-path}/plan.md`
...
2. Read codebase docs; if stale/missing: run `skill:scout` to search codebase
...
7. **Context reminder:** `skill:cook {absolute-plan-path}/plan.md`
...
7. **Context reminder:** `skill:cook --parallel {absolute-plan-path}/plan.md`
...
**Invocation:** Run `skill:plan red-team {plan-directory-path}`.
```
skill:plan red-team {plan-directory-path}
```
...
| `auto` | Run `skill:plan validate {plan-directory-path}` |
| `off` | Skip validation |

**Invocation (when prompt mode, user says yes):** Run:
```
skill:plan validate {plan-directory-path}
```
...
| Mode | Cook Command |
|------|-----------------------------|
| fast | `skill:cook --auto {path}/plan.md` |
| hard | `skill:cook {path}/plan.md` |
| parallel | `skill:cook --parallel {path}/plan.md` |
| two | `skill:cook {path}/plan.md` |
...
After creating: `node .gemini/scripts/set-active-plan.cjs {plan-dir}`
