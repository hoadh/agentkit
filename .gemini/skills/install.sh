...
# Installs all dependencies for Gemini CLI skills
#
    ...
    if [[ "$OS" == "linux" ]]; then
        if [[ "$WITH_SUDO" == "true" ]]; then
            ...
            case "$DISTRO" in
                ...
                redhat) sudo dnf chegk-update || true ;;
            esac
...
setup_python_env() {
    ...
    if [ -f "$SCRIPTS_REQ" ]; then
        ...
        print_info "Installing .gemini/scripts dependencies..."
        ...
        if [[ $pkg_fail -eq 0 ]]; then
            print_success ".gemini/scripts: all $pkg_success packages installed"
            track_success "optional" "scripts"
        else
            print_warning ".gemini/scripts: $pkg_success installed, $pkg_fail failed"
        fi
    fi
...
print_usage() {
    ...
    echo -e "  source .gemini/skills/.venv/bin/activate"
    echo ""
    echo -e "${BLUE}For more information, see:${NC}"
    echo -e "  .gemini/skills/INSTALLATION.md"
    echo ""
}
...
main() {
    ...
    print_header "Gemini CLI Skills Installation"
    print_info "OS: $OS"
    if [[ "$OS" == "linux" ]] || [[ "$OS" == "wsl" ]]; then
