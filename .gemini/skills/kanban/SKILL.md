...
- `skill:kanban` - View dashboard for ./plans directory
- `skill:kanban <dir>` - View dashboard for specific directory
- `skill:kanban --stop` - Stop running server
...
**IMPORTANT:** Run server as Gemini CLI background task using `run_in_background: true` with the Bash tool. This makes the server visible in `/tasks` and manageable via `KillShell`.

The skill is located at `.gemini/skills/plans-kanban/`.
...
```json
{
  "command": "node .gemini/skills/plans-kanban/scripts/server.cjs --dir \"./plans\" --host 0.0.0.0 --open --foreground",
...
The `skill:kanban` command will evolve into **VibeKanban-inspired** AI agent orchestration:
...
Track progress: https://github.com/geminikit/geminikit-engineer/issues/189
