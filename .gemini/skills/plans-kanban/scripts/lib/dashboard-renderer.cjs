...
function generateCardMeta(plan) {
  ...
  // Issue tag - clickable link to GitHub (uses branch to derive repo, falls back to GeminiKit)
  if (plan.issue) {
    // TODO: Make repo configurable via project settings
    const issueUrl = `https://github.com/GeminiKit/GeminiKit/issues/${plan.issue}`;
