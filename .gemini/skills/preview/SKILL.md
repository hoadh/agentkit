...
### View Mode
- `skill:preview <file.md>` - View markdown file in novel-reader UI
- `skill:preview <directory/>` - Browse directory contents
- `skill:preview --stop` - Stop running server

### Generation Mode (Markdown)
- `skill:preview --explain <topic>` - Generate visual explanation (ASCII + Mermaid + prose)
- `skill:preview --slides <topic>` - Generate presentation slides (one concept per slide)
- `skill:preview --diagram <topic>` - Generate focused diagram (ASCII + Mermaid)
- `skill:preview --ascii <topic>` - Generate ASCII-only diagram (terminal-friendly)

### Generation Mode (HTML)
- `skill:preview --html --explain <topic>` - Self-contained HTML explanation
- `skill:preview --html --slides <topic>` - Magazine-quality HTML slide deck
- `skill:preview --html --diagram <topic>` - HTML diagram with zoom controls
- `skill:preview --html --diff [ref]` - Visual diff review
- `skill:preview --html --plan-review [plan-file]` - Plan vs codebase comparison
- `skill:preview --html --recap [timeframe]` - Project context snapshot
...
| Flag without topic | Ask user: "Please provide a topic: `skill:preview --explain <topic>`" |
...
| Server startup failure | Check if port in use, try `skill:preview --stop` first |
...
Use `skill:mermaidjs-v11` skill for Mermaid syntax validation.
...
- For `--slides`: consider invoking `skill:ui-ux-pro-max` for richer style selection
