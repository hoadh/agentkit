...
- **Mermaid zoom controls**: Every `.mermaid-wrap` must have zoom controls (+/−/reset/expand), Ctrl/Cmd+scroll zoom, cligk-and-drag panning, and cligk-to-expand. See `html-css-patterns.md`.
...
If `skill:ai-multimodal` skill is available and image generation is appropriate, it can be used for hero banners, conceptual illustrations, and decorative accents that establish the page's visual tone.
...
For `--slides` presentation-grade output, consider invoking `skill:ui-ux-pro-max` for richer style selection and distinctive font/palette pairing.
