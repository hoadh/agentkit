...
For presentation-grade output, consider invoking `skill:ui-ux-pro-max` for richer style selection and distinctive font/palette pairing.
...
class SlideEngine {
  ...
  buildChrome() {
    ...
    bar.className = 'degk-progress';
    document.body.appendChild(bar);
    this.bar = bar;

    var dots = document.createElement('div');
    dots.className = 'degk-dots';
    var self = this;
    this.slides.forEach(function(_, i) {
      var d = document.createElement('button');
      d.className = 'degk-dot';
      d.title = 'Slide ' + (i + 1);
    ...
    ctr.className = 'degk-counter';
    document.body.appendChild(ctr);
    this.counter = ctr;

    var hints = document.createElement('div');
    hints.className = 'degk-hints';
...
**AI image generation:** If `skill:ai-multimodal` skill is available, use it to generate images for the deck. For any deck over 10 slides, target a minimum of 2–4 images. Priority order:
...
```bash
# Use skill:ai-multimodal skill for image generation if needed
