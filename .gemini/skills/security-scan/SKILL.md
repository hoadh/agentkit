...
Lightweight security scanner using Gemini's reasoning + shell tools. No external dependencies required.

## Usage

```
skill:security-scan              # Full scan of current project
skill:security-scan --secrets-only   # Only secret/credential detection
skill:security-scan --deps-only      # Only dependency audit
skill:security-scan src/api/         # Scan specific directory
```
...
| Code patterns | Grep + Gemini analysis | Medium | `references/vulnerability-patterns.md` |
...
- Use Gemini reasoning to determine if it's a real vulnerability or false positive
