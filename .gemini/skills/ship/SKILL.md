...
Step 8:  Journal          → Write technical journal via skill:journal
Step 9:  Docs update      → Update project docs via skill:docs update (official only)
...
User says `skill:ship` → run full pipeline → output PR URL.
User says `skill:ship beta` → ship to dev branch with lighter pipeline.
User says `skill:ship official` → ship to main with full docs + journal.
