...
2. Add `.gemini-plugin/marketplace.json` with plugin definitions
...
Uses existing git credential helpers. If `git clone` works in terminal, it works in Gemini CLI.
...
Add to `.gemini/settings.json` in your repo:

```json
{
  "extraKnownMarketplaces": {
    "company-tools": {
      "source": { "source": "github", "repo": "your-org/gemini-plugins" }
    }
  }
}
```
