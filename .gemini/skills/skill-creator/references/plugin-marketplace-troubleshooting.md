...
- `.gemini-plugin/marketplace.json` exists at specified path?
- JSON syntax valid? Run `gemini plugin validate .` or `/plugin validate .`
- Private repo — do you have access permissions?

## Validation Errors

Run `gemini plugin validate .` from marketplace directory. Common errors:

| Error | Cause | Fix |
|-------|-------|-----|
| `File not found: .gemini-plugin/marketplace.json` | Missing manifest | Create with required fields |
...
- Use `${GEMINI_PLUGIN_ROOT}` in hooks/MCP configs for cache-aware paths
- See [Plugin caching docs](https://code.gemini.com/docs/en/plugins-reference.md#plugin-caching-and-file-resolution)
