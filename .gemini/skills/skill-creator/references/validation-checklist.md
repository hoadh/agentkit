...
- [ ] Env vars: respects hierarchy `process.env` > `$HOME/.gemini/skills/${SKILL}/.env` (global) > `$HOME/.gemini/skills/.env` (global) > `$HOME/.gemini/.env` (global) > `./.gemini/skills/${SKILL}/.env` (cwd) > `./.gemini/skills/.env` (cwd) > `./.gemini/.env` (cwd)
