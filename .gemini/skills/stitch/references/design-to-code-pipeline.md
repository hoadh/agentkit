...
Orchestration workflows chaining Stitch design generation with GeminiKit implementation skills.

## Basic Pipeline

```
Prompt -> Stitch Generate -> Export HTML -> Gemini Implements Components
```
...
# 4. Activate gk-frontend-design to implement React components from the spec
```
...
|-------------|-------|-------------|
| React/Vue/Svelte components | `gk-frontend-design` | Converts Tailwind HTML to framework components |
| Full page with style guide | `gk-ui-ux-pro-max` | Builds complete layouts with design system |
| Design token extraction | `gk-ui-styling` | Extracts shadcn/Tailwind tokens from DESIGN.md |

## Handoff Protocol

1. `gk-stitch` exports artifacts to `./stitch-exports/` (or plan directory)
...
# -> Skip Stitch, use gk-ui-ux-pro-max for text-based design instead
