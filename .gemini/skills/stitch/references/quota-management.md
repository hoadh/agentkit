...
Stitch SDK has no programmatic quota check endpoint. GeminiKit tracks locally:

**File:** `~/.geminikit/.stitch-quota.json`
...
2. Skill prints: "Daily quota exhausted. Use gk-ui-ux-pro-max as fallback."
3. Activate `gk-ui-ux-pro-max` with the same design prompt
...
Local tracking can drift if user generates designs outside GeminiKit (via Stitch web UI or other tools). If you hit `RATE_LIMITED` error despite local tracker showing credits available:
