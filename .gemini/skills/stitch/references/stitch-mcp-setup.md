...
Three options for connecting Google Stitch as an MCP server with Gemini CLI.
...
Restart Gemini CLI. You should see Stitch tools available:
...
| Tools not appearing | Restart Gemini CLI after config change |
...
For Gemini CLI CLI: use `.gemini/.mcp.json` in project root (preferred for GeminiKit).
