 ...
 *
 * Tracks usage in ~/.geminikit/.stitch-quota.json.
...
const QUOTA_DIR = path.join(os.homedir(), ".geminikit");
...
function check(): void {
  ...
  if (remaining <= 0) {
    console.error("[X] Daily quota exhausted. Use gk-ui-ux-pro-max as fallback.");
    process.exit(2);
