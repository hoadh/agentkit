...
# Agent Teams - GK-Native Orchestration Engine

Coordinate multiple independent Gemini CLI sessions. Each teammate has own context window, loads project context (GEMINI.md, skills, agents), communicates via shared task list and messaging.

**Requires:** Agent Teams enabled. Set `GEMINI_CLI_EXPERIMENTAL_AGENT_TEAMS=1` in settings.json env.
...
**Model requirement:** All teammates must run gemini-1.5-pro (Agent Teams constraint).

## Usage

```
skill:team <template> <context> [flags]
```

**Templates:** `skill:research`, `skill:cook`, `skill:code-review`, `skill:gk-debug`
...
3. If the call returns an ERROR or is unrecognized: **STOP. Tell user:** "Agent Teams requires `GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS=1` in settings.json. Team mode is not available."
4. Do NOT fall back to subagents. `skill:team` MUST use Agent Teams or abort.
...
```
Agent(
  subagent_type: "researcher" | "fullstagk-developer" | "code-reviewer" | "debugger" | "tester" | ...,
  description: "short task summary",
  prompt: "full instructions + CK Context Block",
  model: "gemini-1.5-pro",          # Required for Agent Teams teammates
...
## ON `skill:team research <topic>` [--researchers N]:

*Wraps skill:research skill -- scope, gather, analyze, report.*
...
4. **SPAWN** teammates x N via `Agent` tool:
   - `subagent_type: "researcher"`, `model: "gemini-1.5-pro"`
...
11. **JOURNAL**: Run `skill:journal` to write a concise technical journal entry upon completion

---

## ON `skill:team cook <plan-path-or-description>` [--devs N]:

*Wraps skill:cook skill -- plan, code, test, review, finalize.*
...
4. **SPAWN** developer teammates x N via `Agent` tool:
   - `subagent_type: "fullstagk-developer"`, `model: "gemini-1.5-pro"`
...
5. **MONITOR** dev completion via TaskCompleted events:
   ...
   - Spawn tester: `Agent(subagent_type: "tester", model: "gemini-1.5-pro", name: "tester")`
...
7. **DOCS SYNC EVAL** (MANDATORY for cook -- from skill:cook finalize):
   ```
...
## ON `skill:team review <scope>` [--reviewers N]:

*Wraps skill:code-review skill -- scout, review, synthesize with evidence gates.*
...
4. **SPAWN** reviewers x N via `Agent` tool:
   - `subagent_type: "code-reviewer"`, `model: "gemini-1.5-pro"`
   - `run_in_background: true`
   - `name: "reviewer-{N}"`
   - Prompt: task description + CK Context Block
...
10. **JOURNAL**: Run `skill:journal` to write a concise technical journal entry upon completion

---

## ON `skill:team debug <issue>` [--debuggers N]:

*Wraps skill:fix skill -- root-cause-first, adversarial hypotheses, disprove to converge.*
...
4. **SPAWN** debugger teammates x N via `Agent` tool:
   - `subagent_type: "debugger"`, `model: "gemini-1.5-pro"`
   - `run_in_background: true`
   - `name: "debugger-{N}"`
   - Prompt: task description + CK Context Block
...
Teammates with `memory: project` in their agent definition retain learnings across team sessions. Memory persists in `.gemini/agent-memory/<name>/` (gitignored). Useful for:
...
Memory persists after team cleanup -- it's in `.gemini/agent-memory/`, not `~/.gemini/teams/`.
...
If unresponsive: close terminal or kill session. Clean orphaned configs at `~/.gemini/teams/` manually.
...
See `.gemini/rules/team-coordination-rules.md` for teammate behavior rules.
