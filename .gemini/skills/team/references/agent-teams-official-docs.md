...
> **Canonical source:** https://code.gemini.com/docs/en/agent-teams
> **Version captured:** Gemini CLI v2.1.80 (March 2026)
> **Update policy:** Re-fetch canonical URL when Gemini CLI releases new Agent Teams features.

This is a **self-contained knowledge base** -- AI agents should NOT need to re-fetch the URL.

## Overview

Agent Teams coordinate multiple Gemini CLI instances working together. One session acts as the team lead, coordinating work, assigning tasks, and synthesizing results. Teammates work independently, each in its own context window, and communicate directly with each other.
...
| **Context** | Own 200K-token window; results return to caller | Own full Gemini CLI instance + context |
...
```json
{ "env": { "GEMINI_CODE_EXPERIMENTAL_AGENT_TEAMS": "1" } }
```
...
Two paths:
1. **You request**: describe task + ask for agent team. Gemini creates based on instructions.
2. **Gemini proposes**: suggests team if task benefits from parallel work.

Both require your confirmation. Gemini won't create a team without approval.
...
| **Teammates** | Separate Gemini CLI instances with own context windows |
| **Task list** | Shared work items at `~/.gemini/tasks/{team-name}/` |
| **Mailbox** | Messaging system for inter-agent communication |

Storage:
- **Team config**: `~/.gemini/teams/{team-name}/config.json` (members array with name, agent ID, type)
...
**Built-in subagent types:** `general-purpose`, `Explore`, `Plan`, `researcher`, `fullstagk-developer`, `code-reviewer`, `debugger`, `tester`, `planner`, `docs-manager`, `brainstormer`, and more.

**Custom subagents:** Define in `.gemini/agents/` with frontmatter (name, description, tools, model).
...
|-------|----------|-----------------|
| `user` | `~/.gemini/agent-memory/<name>/` | All projects |
| `project` | `.gemini/agent-memory/<name>/` | Sessions in same project |
...
Each teammate loads: GEMINI.md, MCP servers, skills, agents. Receives spawn prompt from lead. Lead's conversation history does NOT carry over.
