 ...
 *   --worktree-root <path> Explicit worktree directory (Gemini's decision)
...
// --worktree-root: explicit override for worktree location (Gemini's decision)
...
// 1. Explicit --worktree-root flag (Gemini's decision)
...
function getWorktreeRoot(gitRoot, isMonorepo, explicitRoot = null) {
  // Priority 0: Explicit --worktree-root flag (Gemini's decision)
  if (explicitRoot) {
...
function cmdCreate() {
  ...
  // explicitWorktreeRoot comes from --worktree-root flag (Gemini's decision)
